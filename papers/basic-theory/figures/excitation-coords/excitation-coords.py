            ...
            for inner_col in range(inner_cols):
                ...
                if inner_col == 2:
                    ...
                    if inner_row == 0:
                        ...
                        ax.text(0.7*np.cos(r_theta+d_theta), 0.7*np.sin(r_theta+d_theta),'$\mathbf{\hat{r}}$', ha='center', va='center', transform=ax.transData, clip_on=False)
                        ax.text(0.7*np.cos(r_theta-d_theta), 0.7*np.sin(r_theta-d_theta),'$\\theta$', ha='center', va='center', transform=ax.transData, clip_on=False)                        
                    ...
                    if inner_row == 1:
                        ...
                        ax.axis('on')
                        ax.arrow(0, 0, 0.4*np.cos(r_theta), 0.4*np.sin(r_theta), head_width=0.05, fc='k', transform=ax.transData)
                        ax.text(0.75*np.cos(r_theta), 0.75*np.sin(r_theta),'$\mathbf{\hat{r}}$', ha='center', va='center', transform=ax.transData, clip_on=False)
                        s_theta = -np.pi/4
                ...
                if inner_col == 0:
                    ...
                    if inner_row == 0:
                        ...
                        ax.text(-0.5, 0.2,'$\mathbf{E}_{\\textrm{bfp}}$', ha='center', va='center', transform=ax.transData, clip_on=False)
                    if inner_row == 1:
                        r_theta = np.pi/6
                        ax.arrow(0, 0, 0.4*np.cos(r_theta), 0.4*np.sin(r_theta), head_width=0.05, fc='k', transform=ax.transData)
                        ax.text(0.6*np.cos(r_theta), -0.2,'$\mathbf{E}_{\\textrm{bfp}}$', ha='center', va='center', transform=ax.transData, clip_on=False)
                        ...
                        l3 = lines.Line2D(0.15*np.cos(tt), 0.15*np.sin(tt), c='k', lw=0.5, transform=ax.transData, zorder=3)
                        ax.text(0.7*np.cos(np.pi/3), 0.7*np.sin(np.pi/3),'$\\phi_{\\textrm{pol}}$', ha='center', va='center', transform=ax.transData, clip_on=False)
                        f.lines.extend([l1, l3])

                if inner_row == 0 and inner_col == 2:
                    # Draw alpha
                    l1 = lines.Line2D([0,-2,-2,0,0.5], [0,0,1,0,0], c='k', lw=0.5, transform=ax.transData, zorder=3, ls=':')
                    ...
                    ax.text(-2.3,0.5,'$\\sin\\alpha \\approx \\alpha$', ha='right', va='center', transform=ax.transData)

                    # Draw theta
                    tt = np.linspace(-5*np.pi/6,-np.pi,20)
                    l5 = lines.Line2D(0.3*np.cos(tt), 0.3*np.sin(tt), c='k', lw=0.5, transform=ax.transData, zorder=3)
                    f.lines.extend([l1, l4, l5])
...
f.savefig('excitation-coords.pdf', dpi=500, bbox_inches='tight')
