...
\begin{document}
  \begin{tikzpicture}[auto, node distance=2.5cm,>={Latex[length=2mm]}]
    ...
    \node [block,  right of=sad, node distance=5.5cm] (ads) {$H_\ell^m(\rd - \ro)$\\ Dipole angular\\ transfer function};
    \node [block,  left of=sad, node distance=5.5cm] (e) {$\mb{c}(\rd - \ro, \so)$\\ Dipole coherent \\ spread function};
      \node [block, below of=ads, node distance=2.5cm] (sas) {$\mathsf{H}_\ell^m(\bs{\nu})$\\ Dipole spatio-angular\\ transfer function};
      \node [block, below of=sad, node distance=2.5cm] (sds) {$H(\bs{\nu}, \so)$\\ Dipole spatial\\ transfer function};
        \node [block,  left of=sds, node distance=5.5cm] (e2) {$\mb{C}(\bvp,\so)$\\ Dipole coherent \\transfer function};
