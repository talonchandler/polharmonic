...
\begin{document}
  \begin{tikzpicture}[auto, node distance=2cm,>={Latex[length=2mm]}]
    ...
    \node [block, align=center] (sad) {$f(\ro{}, \so)$\\ Spatio-angular\\ density};
    \node [block,  right of=sad, node distance=4cm] (ads) {$F_\ell^m(\ro)$\\ Angular density\\ spectrum};
    \node [block,  right of=ads, node distance=9cm] (irr) {$g(\rd)$\\ Irradiance};
    \node [block, below of=ads, node distance=2.5cm] (sas) {$F_\ell^m(\bs{\nu})$\\ Spatio-angular\\ spectrum};
    \node [block,  right of=sas, node distance=9cm] (is) {$G(\bs{\nu})$\\ Irradiance\\ spectrum};
    \node [block, below of=sad, node distance=2.5cm] (sds) {$F(\bs{\nu}, \so)$\\ Spatial density\\ spectrum};
    ...
    \draw [->] (ads) -- node[text width=4cm, align=center] (cap1) {Angular\\ transfer function\\ $\sum_{\ell m}H_\ell^m(\rd - \ro)F_\ell^m(\ro)$} (irr);
    \draw [->] (sad.north) -- ++(0,1.0cm) -- node[right=2.cm, above, align=center] {Orientation-dependent\\ point spread function\\ $\int_{\mbb{S}^2}d\so\, \int_{\mbb{R}^2} d\ro\, h(\rd - \ro, \so)f(\ro, \so)$} ++(13cm,0) --  (irr.north);
    \draw [->] (sds.south) -- ++(0,-1.0cm) -- node[right=1.95cm, above, align=center] {Orientation-dependent\\ optical transfer function\\ $\int_{\mbb{S}^2}d\so\, H(\bs{\nu}, \so)F(\bs{\nu}, \so)$} ++(13cm,0) -- (is.south);
    \draw [->] (sas) -- node[text width=4cm, align=center] {Spatio-angular\\ transfer function\\ $\sum_{\ell m}H_\ell^m(\bs{\nu})F_\ell^m(\bs{\nu})$} (is);    
