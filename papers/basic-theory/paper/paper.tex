...
measuring ensembles of fluorescent molecules that label specific proteins and
biologically important molecules. While most fluorescence microscopy experiments
are designed to measure only the spatial distribution of fluorophores, a growing
number of experiments seek to measure both the spatial and angular distributions
of fluorophores by the use of polarizers \cite{vrabioiu2006, mattheyses2010,
...
microscopes the monopole approximation is assumed. % As previously mentioned,
...
microscopy: (1) spatial ensemble imaging where fluorophores are imaged as
ensembles and the goal is to find their concentration as a function of position
in the sample, (2) the spatio-angular ensemble imaging where fluorophores are
imaged as ensembles and the goal is to find their concentration and orientation
as a function of position in the sample, and (3) SMLM imaging where fluorophores
are sparse in the sample and the goal is to find the position and orientation of
each fluorophore. We briefly review how these three subfields use the monopole
...
fluorescence microscopy data because they only model monopole emitters.
...
In this work we place these three subfields on a common theoretical footing. We
begin in section \ref{sec:theory} by formulating the imaging problem
...
% We have chosen the symbol $\taup$ as the coordinate for the coherent
...
\end{align}
% This integral will be difficult to evaluate unless we change coordinates to
% exploit the axis of symmetry $\mh{s}'$. The key idea is that
% $f_{\text{(cone)}}(\mh{s}; \mh{s}', \Delta)$ is a function of
% $\mh{s}\cdot\mh{s}'$ so it is a one dimensional function on the domain [-1,1]
% that can be expanded in terms of Legendre polynomials (a Fourier-Legendre
% series)
% \begin{align}
%   f_{\text{(cone)}}(\mh{s}\cdot\mh{s}', \Delta) = \sum_{\ell = 0}^{\infty}F_{\ell\text{(cone)}}(\mh{s}\cdot\mh{s}', \Delta)P_{\ell}(\mh{s}\cdot\mh{s}').
% \end{align}
% We can find the coefficients $F_{\ell\text{(cone)}}(\mh{s}\cdot\mh{s}', \Delta)$
% using Fourier's trick---multiply both sides by $P_{\ell}(\mh{s}\cdot\mh{s}')$,
% integrate over [-1,1], and exploit orthogonality
% $\int_{-1}^{1}dx\,P_{\ell}(x)P_{\ell'}(x) = \frac{2}{2\ell +
%   1}\delta_{\ell\ell'}$ to find that
% \begin{align}
%   F_{\ell\text{(cone)}}(x, \Delta) = \frac{2\ell + 1}{2}\int_{-1}^{1}dx\, f_{\text{(cone)}}(x, \Delta)P_\ell(x). \label{eq:legendre}
% \end{align}
% Plugging Eq. \eqref{eq:doublecone} into Eq. \eqref{eq:legendre} yields
% \begin{align}
%   F_{\ell\text{(cone)}}(x, \Delta) = \frac{2\ell + 1}{4\pi(1-\cos\Delta)}\int^{1}_{\cos\Delta}dx\, P_\ell(x). 
% \end{align}
...
% $m=-1$, not an inverse Legendre polynomial.
