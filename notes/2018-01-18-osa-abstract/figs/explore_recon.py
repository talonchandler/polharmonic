...
idx1 = [0, 1, 2, 3]
idx2 = [4, 5, 6, 7]
for i in itertools.permutations(idx1):
    for j in itertools.permutations(idx2):
        print(i+j)
        ind = list(i+j)
        file_out= str(i+j).replace(', ', '')[1:-1] + '.png'
        recon_roi(name='explore-cal3', xs=50, ys=50, zs=20, x0=180, y0=545, z0=150, 
                  recon_mask_threshold=0.6, d=130, skip=2,
                  dpi=800, mag=1, note='zoomed out version of roi1',
                  idx = ind, file_out=file_out, second_cal=3)

        recon_roi(name='explore-cal24', xs=50, ys=50, zs=20, x0=180, y0=545, z0=150, 
                  recon_mask_threshold=0.6, d=130, skip=2,
                  dpi=800, mag=1, note='zoomed out version of roi1',
                  idx = ind, file_out=file_out, second_cal=2.4)

        recon_roi(name='explore-cal15', xs=50, ys=50, zs=20, x0=180, y0=545, z0=150, 
                  recon_mask_threshold=0.6, d=130, skip=2,
                  dpi=800, mag=1, note='zoomed out version of roi1',
                  idx = ind, file_out=file_out, second_cal=1.5)          
