...
\begin{align}
  a(r_o') &= \int_0^{\theta_{\text{max}}}d\theta\ \sqrt{\cos\theta}\sin\theta(1+\cos\theta)J_0(k_b r_o'\sin\theta f_o/f_t),\\%\ \text{exp}\left\{ik_bz_o[1-(1/2)(f_o/f_t)^2\sin^2\theta]\right\},\\
  b(r_o') &= \int_0^{\theta_{\text{max}}}d\theta\ \sqrt{\cos\theta}\sin^2\theta J_1(k_b r_o'\sin\theta f_o/f_t),\\%\ \text{exp}\left\{ik_bz_o[1-(1/2)(f_o/f_t)^2\sin^2\theta]\right\},\\
  c(r_o') &= \int_0^{\theta_{\text{max}}}d\theta\ \sqrt{\cos\theta}\sin\theta(1-\cos\theta)J_2(k_b r_o'\sin\theta f_o/f_t).%\ \text{exp}\left\{ik_bz_o[1-(1/2)(f_o/f_t)^2\sin^2\theta]\right\}.
\end{align}
...
% \begin{align}
%   \tilde{\mb{e}}_d(\rd{}', \so{}) &= \sum_{l=0}^{\infty} \sum_{m=-l}^l \int_{\mathbb{R}^2}d\bs{\nu}{}\, \mb{E}_l^m(\bs{\nu})y_l^m(\so{})\thinspace\me^{i 2\pi \rd{}'\cdot\bs{\nu}},\label{eq:csfexp}
% \end{align}
% where $\mb{E}_l^m(\bs{\nu})$ is the spatio-angular spectrum of the CSF given
% by
% \begin{align}
%   \mb{E}_l^m(\bs{\nu}) \equiv \int_{\mathbb{S}^2}d\so{}\int_{\mathbb{R}^2}d\rd{}'\, \tilde{\mb{e}}_d(\rd{}', \so{}) y_l^m(\so{})\thinspace\me^{-i 2\pi \rd{}'\cdot\bs{\nu}}.\label{eq:csfdef2}
% \end{align}
...
\begin{tikzpicture}[auto, node distance=3cm,>=latex]
    ...
    \node [block, align=center] (csf) {CSF\\ $\tilde{\mb{e}}'_d(\ro{}', \so{})$};
    ...
    \draw [<->] (psf) -- node[below] {$\mathcal{F}_{\mathbb{R}^2\times\mathbb{S}^2}$} (otf);
    \draw [->] (csf) -- node[name=v, text width=4cm, align=center, left] {Detector\\ $|\mb{\tilde{e}}'_d(\ro{}', \so{})|^2_{\mathbb{R}^2}$} (psf);
      \end{tikzpicture}
      \centering
      \captionsetup{width=1.0\linewidth}
