...
\begin{align}
  h^{(p)}(\rd{}', \so{}) &= {h_0^0}^{(p)}(\rd{}')y_0^0(\so{}) + {h_0^2}^{(p)}(\rd{}')y_0^2(\so{}),\\
  {h_0^0}^{(p)}(\rd{}') &\equiv {a^{(p)}}^2(r_d') + 2{b^{(p)}}^2(r_d'),\\
  {h_0^2}^{(p)}(\rd{}') &\equiv \frac{1}{\sqrt{5}}\left[- {a^{(p)}}^2(r_d') + 4{b^{(p)}}^2(r_d')\right].
\end{align}
...
\begin{align}
  ...
  {H_0^0}^{(p)}(\nu) &\equiv \frac{A^{(p)}(\nu) + 2B^{(p)}(\nu)}{1 + \text{NA}^2/(2n_o^2)},\\
  {H_2^0}^{(p)}(\nu) &\equiv \frac{-A^{(p)}(\nu) + 4B^{(p)}}{\sqrt{5}\, [1 + \text{NA}^2/(2n_o^2)]}.
\end{align}
where
\begin{align}
  {A}^{(p)}(\nu) &= \frac{2}{\pi}\left[\arccos\left(\frac{\nu}{2\nu_o}\right) - \frac{\nu}{2\nu_o}\sqrt{1 - \left(\frac{\nu}{2\nu_o}\right)^2}\right]\Pi\left(\frac{\nu}{2\nu_o}\right),\\
  {B}^{(p)}(\nu) &= \frac{\text{NA}^2}{2\pi n_o^2}\left[\left(1 - \frac{\nu^2}{\nu_o^2}\right)\arccos\left(\frac{\nu}{2\nu_o}\right) + \left(1 + \frac{\nu^2}{2\nu_o^2}\right)\frac{\nu}{2\nu_o}\sqrt{1 - \left(\frac{\nu}{2\nu_o}\right)^2}\right]\Pi\left(\frac{\nu}{2\nu_o}\right).
\end{align}
...
no longer be true when we leave the paraxial approximation or add polarizing
filters to the system. Also, notice that ${A}^{(p)}(\nu)$ is the spatial OTF for
an incoherent imaging system under the paraxial and scalar approximations
\cite{goodman1996}.
...
${B}^{(p)}(\nu)$ disappears and we recover the spatial OTF from the
...
correction is proportional to $\frac{\text{NA}^2}{n_o^2}$, so it is a small term
...
\end{align}
The main result is that the spatial OTF for any dipole distribution is a linear
combination of the OTFs for the spherical harmonics. I am partially hesitant to
refer to $H_{\mh{z}}(\nu)$ as an OTF because it is object dependent, but the
vocabulary is fine if it is understood that the sample only contains
$z$-oriented dipoles. For general distributions, the complete spatio-angular OTF
should be used instead of deriving the transfer function for every individual
fluorophore.

Fig. \ref{fig:para} shows the spatio-angular OTF for a microscope with
$\text{NA}=0.8$ and $n=1.33$. The most important feature of this figure is that
${H_0^0}^{(p)}(\nu)$ is a low-pass filter compared to ${H_0^2}^{(p)}(\nu)$ with
a maximum difference of $\sim 7\%$ (with larger difference for higher NA). If the difference between
${H_0^0}^{(p)}(\nu)$ and ${H_0^2}^{(p)}(\nu)$ is not completely obscured by
noise, we will be able to exploit the difference to estimate the out-of-plane
angle of dipoles with a single image (putting aside the ill-posedness of the
problem).
...
\begin{figure}[h]
 ...
 \centering
   \centering
   \includegraphics[width = 1.\textwidth]{../calculations/psfs.pdf}
   \caption{\textbf{Left:} Paraxial spatio-angular point spread function for a
     single-view fluorescence microscope with NA=0.8 and $n_o$=1.33. The solid
     line is the PSF for $y_0^0$ distributions and the dashed line is the PSF
     for $y_2^0$ distributions. The dotted line is the $y_2^0$ PSF normalized
     for comparison with the $y_0^0$ PSF. The $y_2^0$ distributions includes
     ``negative'' fluorophores, so it gives rise to a negative PSF.
     \textbf{Right:} Numerical paraxial spatio-angular optical transfer function
     for the same microscope. The $y_0^0$ term has a comparatively low-pass
     response while the $y_2^0$ term has a comparatively high-pass response.}
   \label{fig:para}
\end{figure}
...
This results in
\begin{align}
  B^{(p)}(\nu)&=\frac{4\text{NA}^2}{\pi\nu_o^4n_o^2}\Bigg[\int\limits_0^a d\tau\left(\frac{\tau^3}{2}\left(\frac{\nu}{2\nu_o}\right)\sqrt{1 - \left(\frac{\nu}{2\nu_o}\right)^2} + \frac{\tau^3}{2}\arccos\left(\frac{\nu}{2a}\right) - \frac{\tau\nu^2}{4}\arccos\left(\frac{\nu}{2\nu_o}\right)\right)\nonumber \\ &\hspace{7em}- \int\limits_0^{\nu/2}d\tau_x \left(\tau_x^3 - \frac{\tau_x\nu^2}{4}\right)\frac{2 \nu_o}{\nu}\sqrt{1 - \left(\frac{\nu}{2\nu_o}\right)^2}\Bigg]\Pi\left(\frac{\nu}{2\nu_o}\right),
\end{align}
\begin{align}
  B^{(p)}(\nu) &=  \frac{4\text{NA}^2}{\pi\nu_o^4n_o^2}\Bigg[\left(\frac{\nu_o^4}{8}\left(\frac{\nu}{2\nu_o}\right)\sqrt{1 - \left(\frac{\nu}{2\nu_o}\right)^2} + \frac{\nu_o^4}{8}\arccos\left(\frac{\nu}{2\nu_o}\right)- \frac{\nu_o^2\nu^2}{8}\arccos\left(\frac{\nu}{2\nu_o}\right)\right)\nonumber \\ &\hspace{7em} - \left(\frac{\nu^4}{64} - \frac{\nu^4}{32}\right)\frac{2\nu_o}{\nu}\sqrt{1 - \left(\frac{\nu}{2\nu_o}\right)^2}\Bigg]\Pi\left(\frac{\nu}{2\nu_o}\right),\\
  B^{(p)}(\nu) &=  \frac{4\text{NA}^2}{\pi\nu_o^4n_o^2}\Bigg[\frac{\nu\nu_o^3}{16}\sqrt{1 - \left(\frac{\nu}{2\nu_o}\right)^2} + \frac{\nu_o^4}{8}\arccos\left(\frac{\nu}{2\nu_o}\right) - \frac{\nu_o^2\nu^2}{8}\arccos\left(\frac{\nu}{2\nu_o}\right) + \frac{\nu^3\nu_o}{32}\sqrt{1 - \left(\frac{\nu}{2\nu_o}\right)^2}\Bigg]\Pi\left(\frac{\nu}{2\nu_o}\right),\\
  B^{(p)}(\nu) &= \frac{\text{NA}^2}{2\pi n_o^2}\left[\frac{\nu}{2\nu_o}\sqrt{1 - \left(\frac{\nu}{2a}\right)^2} + \arccos\left(\frac{\nu}{2\nu_o}\right) - \frac{\nu^2}{\nu_o^2}\arccos\left(\frac{\nu}{2\nu_o}\right) + \frac{\nu^3}{4\nu_o}\sqrt{1 - \left(\frac{\nu}{2\nu_o}\right)^2}\right]\Pi\left(\frac{\nu}{2\nu_o}\right),\\
  B^{(p)}(\nu) &= \frac{\text{NA}^2}{2\pi n_o^2}\left[\left(1 - \frac{\nu^2}{\nu_o^2}\right)\arccos\left(\frac{\nu}{2\nu_o}\right) + \left(1 + \frac{\nu^2}{2\nu_o^2}\right)\frac{\nu}{2\nu_o} \sqrt{1 - \left(\frac{\nu}{2\nu_o}\right)^2}\right]\Pi\left(\frac{\nu}{2a}\right).
\end{align}
